...
namespace IntentProcessing.Controllers
{
    [BotAuthentication]
    public class IntentsController : ApiController
    {
        ...
        {
            var properties = new Dictionary<string, string> { {"Page Name","IntentsController" }, {"Method Name","Post" },
            ...
            var aiproperties = new Dictionary<string, string> { {"Page Name","IntentsController" }, {"Method Name","Post" },
            ...
            try
            {
                ...
                var insightproperties = new Dictionary<string, string> { {"Page Name","IntentsController" }, {"Method Name","Post" },
            ...
            if (data.Count == 3)
            {
                ...
                for (int i = 0; i < posTags.Nodes.Count; i++)
                {
                    if (posTags.Nodes[i] == "NNP")
                    {
                        ...
                        botOutputString += tokenData.Tokens[i].RawToken + " is Noun" + " ~ ";
                    }
                    else if (posTags.Nodes[i] == "VBG" || posTags.Nodes[i] == "VB")
                    {
                        botOutputString += tokenData.Tokens[i].RawToken + " is Verb" + " ~ ";
                    }
                    else if (posTags.Nodes[i] == "WRB")
                    {
                        botOutputString += tokenData.Tokens[i].RawToken + " is Adverb" + " ~ ";
                    }
                    else if (posTags.Nodes[i] == "WP")
                    {
                        botOutputString += tokenData.Tokens[i].RawToken + " is Pronoun" + " ~ ";
                    }
                    else if (posTags.Nodes[i] == "JJ" || posTags.Nodes[i] == "JJR" || posTags.Nodes[i] == "JJS")
                    {
                        botOutputString += tokenData.Tokens[i].RawToken + " is Adjective" + " ~ ";
                    }
                    else if (posTags.Nodes[i] == "IN")
                    {
                        botOutputString += tokenData.Tokens[i].RawToken + " is Preposition" + " ~ ";
                    }
                }

                botOutputString = botOutputString!=""?"Speech and Natural Language Processing ~" + botOutputString:"";
            ...
            var luisOutputString = "Intent and Language Understanding Intelligence Service Processing results are ~";
            ...
            var insightsproperties = new Dictionary<string, string> { {"Page Name","IntentsController" }, {"Method Name","Post" },
                ...
                foreach (var entity in luisResponse.entities)
                {
                    ...
                    if (entity.type.Contains("geography"))
                    {
                        if (!luisOutputString.ToLower().Contains(entity.entity.ToLower()))
                            luisOutputString += entity.type.Replace("builtin.geography.", "") + " : " + entity.entity + " ~ ";
                    }
                    ...
                    {
                        luisOutputString += entity.type + " " + entity.entity + " ~ ";
                    }
                }
            }
            else
            ...
            var appinsightsproperties = new Dictionary<string, string> { {"Page Name","IntentsController" }, {"Method Name","Post" },
            ...
            return botOutputString + " ` " + luisOutputString;
        ...
        private async Task<TResponse> SendRequestAsync<TRequest, TResponse>(HttpMethod httpMethod, string requestUrl, TRequest requestBody = default(TRequest))
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var properties = new Dictionary<string, string> { {"Page Name","IntentsController" }, {"Method Name","SendRequestAsync" },
